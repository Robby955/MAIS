...
**Problem ([MAIS-A4, Problem 5.7](../agendas/A4/MAIS-A4.tex#L424)).** Call the objective $L+\lambda R$ **identifiable at** $(m,n,S,\lambda)$ if its $\mathrm{argmin}$ is nonempty and consists of a single $G$-orbit.

1. Decide identifiability for $(m,n)=(5,2)$, $\lambda=0$, at high sparsity: prove there exists $S_0<1$ such that for all $S\in(S_0,1)$ the $\mathrm{argmin}$ is a single $G$-orbit — concretely, the regular pentagon with its optimal norms and biases — or prove that no such $S_0$ exists.
