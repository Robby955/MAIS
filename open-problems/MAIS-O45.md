...
2. (*Transfer.*) Exhibit $(m,n,S,\lambda)$ and $(d,\alpha,\varepsilon)$ with $\varepsilon<\tfrac12$ such that both $\mathrm{argmin}(L+\lambda R)$ and $\mathrm{argmin}L$ are nonempty, every $(W_\lambda,b_\lambda)\in\mathrm{argmin}(L+\lambda R)$ satisfies $\mathrm{REC}_S(W_\lambda;d,\alpha,\varepsilon)=1$, and some $(W_0,b_0)\in\mathrm{argmin} L$ satisfies $\mathrm{REC}_S(W_0;d,\alpha,\varepsilon)=0$. Or prove no such tuple exists.
