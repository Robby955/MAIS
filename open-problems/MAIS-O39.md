...
**Problem ([MAIS-A3, Problem 4.9](../agendas/A3/MAIS-A3.tex#L298)).** Fix a superposition model $\mathcal S$ and $\lambda>0$. Let $\mathcal M_F=\mathrm{argmin}_{U_{n,M}}F_\lambda$, and let $\mathcal A_G$ be the set of dictionaries $\Psi\in U_{n,M}$ for which some minimizing sequence $(\Psi_r,W_r,b_r)$ for $G_\lambda$ has $\Psi_r\to\Psi$. Compare $\mathcal M_F$ with $\mathcal A_G$.
