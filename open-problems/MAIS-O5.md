...
Parts (a) and (b) are posed for each fixed tuple separately. A point mass means deterministic selection; two explicitly charged sets with their probabilities would establish genuinely random selection. One constraint is proved for the quadratic activation: the selection law is invariant under the outer automorphism group of $G$, so for $G = C_p$ with $p$ prime every frequency (nontrivial character) is key with equal probability. The nearest theorems live in idealized regimes — the maximum-margin characterization of Morwani, Edelman, Oncescu, Zhao, and Kakade [[MEOZK24]](../references/MEOZK24.md) in the $\ell_{2,3}$ norm, or the Taylor-surrogate projected flow of He, Wang, Zhang, Chen, and Yang [[HWZCY26]](../references/HWZCY26.md) — and do not predict this ensemble; the smallest case with genuine competition, $C_5$ at width two with the decay switched off, is already open. Definitions, known results, and starter cases are collected in [MAIS-A5](../agendas/A5/).
